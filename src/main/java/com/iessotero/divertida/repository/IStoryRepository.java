...
public interface IStoryRepository extends JpaRepository<Stories, Long> {
	...
	@Modifying
	@Transactional
	@Query("UPDATE Stories s SET s.categoriesStory.id = :categoryId, s.title = :title, s.description = :description WHERE s.id = :id")
	void updateStory(@Param("id") Long id, @Param("categoryId") Long categoryId, @Param("title") String title,
