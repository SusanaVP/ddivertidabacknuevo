...
public class RiddlesController {
	 ...
	 * @return una respuesta HTTP con el estado OK si se añade correctamente, o
	 ...
	 * @return una respuesta HTTP con el estado OK si se elimina correctamente, o
	 *         INTERNAL_SERVER_ERROR en caso de error.
	 */
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	 ...
	 */
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	@PutMapping("/editRiddle")
	public ResponseEntity<String> editRiddle(@RequestBody Riddles riddle) {
		try {
			this.riddlesService.editRiddle(riddle);
			return new ResponseEntity<>(HttpStatus.OK);
		} catch (Exception e) {
			return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
		}
